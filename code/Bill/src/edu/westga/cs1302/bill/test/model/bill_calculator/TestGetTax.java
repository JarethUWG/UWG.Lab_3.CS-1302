...
class TestGetTax {
	...
	void testWhenPassedEmptyArray() {
		...
		assertEquals(0.0, BillCalculator.getTax(items), 0.001);
	}
	...
	void testWhenPassedOneItemArray() {
		...
		items[0] = item1;
		assertEquals(1.05, BillCalculator.getTax(items), 0.001);
	}
	...
	void testWhenPassedTwoItemArray() {
		...
		items[0] = item1;
		items[1] = item2;
		assertEquals(1.56, BillCalculator.getTax(items), 0.001);
	}
	...
	void testWhenPassedThreeItemArray() {
		...
		items[1] = item2;
		items[2] = item3;
		assertEquals(3.1, BillCalculator.getTax(items), 0.001);
	}
